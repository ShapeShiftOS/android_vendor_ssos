function __print_ssos_functions_help() {
cat <<EOF
Additional ShapeShiftOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- ssosgerrit:    A Git wrapper that fetches/pushes patch from/to ShapeShiftOS Gerrit Review.
- ssosrebase:    Rebase a Gerrit change and push it again.
- ssosremote:    Add git remote for ShapeShiftOS Gerrit Review.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the ShapeShift model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch ssos_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.ssos.device | grep -q "$SSOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SSOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function ssosremote()
{
    ...
    then
        ...
        local PFX="ShapeShiftOS/"
    else
        local PROJECT=$REMOTE
    fi

    local SSOS_USER=$(git config --get review.review.ssos.org.username)
    if [ -z "$SSOS_USER" ]
    then
        git remote add ssos ssh://review.ssos.org:29418/$PFX$PROJECT
    else
        git remote add ssos ssh://$SSOS_USER@review.ssos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'ssos' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.ssos.device | grep -q "$SSOS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        ssosremote
        git push ssos HEAD:refs/heads/'$1'
    fi
    '
}
...
function ssosgerrit() {
    ...
    local user=`git config --get review.review.ssos.org.username`
...
function ssosrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      ssosrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.ssos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.ssos.device | grep -q "$SSOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SSOS_FIXUP_COMMON_OUT ]; then
