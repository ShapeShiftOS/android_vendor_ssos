...
func init() {
	android.RegisterModuleType("ssos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ssosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ssosExpandVariables(ctx, dep_file)
	...
	exCmd := ssosExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "ssos.sbox.textproto"
	desc := "generate"
	name := "generator"
